...
~~*Special characters are not handled well for now, other language than English can use ANSI code (for Europe, utf8 otherwise) this way : `¦` is coded `\xa6`.*~~

**Edit** : updating `reprex` to the dev version solved the issue (specific to windows)
...
reprex::reprex(x = {
	...
	# Method 1 - base R
	dfr_clean <- dfr
	dfr_clean$var1 <- gsub(pattern = "\xa6", replacement = "a", x = dfr$var1)
	dfr_clean[["var2"]] <- gsub(pattern = "\xa6", replacement = "e", x = dfr$var2)
	
	# Method 2 - tidyverse
	suppressPackageStartupMessages( library(dplyr) )
	suppressPackageStartupMessages( library(stringr) )
	dfr %>% mutate(var1 = str_replace_all(string = var1, pattern = "\xa6", replacement = "a"),
		       var2 = str_replace_all(string = var2, pattern = "\xa6", replacement = "e")
	)
}, si = TRUE, venue = "so")
```
